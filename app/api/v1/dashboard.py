...
async def get_dashboard_stats():
    ...
    upcoming_sessions = await Session.filter(start_date__gte=datetime.now().date(), status="scheduled").count()
...
async def get_upcoming_sessions():
    ...
    """
    sessions = (
        await Session.filter(start_date__gte=datetime.now().date(), status="scheduled")
...
async def get_pending_payments():
    ...
    """
    invoices = (
        await Invoice.filter(status__in=["sent", "partial", "overdue"])
