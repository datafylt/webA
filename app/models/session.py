...
class Session(BaseModel, TimestampMixin):
    ...
    location_type = fields.CharField(
        max_length=20, 
        default="in_person",
        description="Type: in_person, online, hybrid"
    )
    ...
    price = fields.DecimalField(max_digits=10, decimal_places=2, null=True, description="Prix (null = prix du programme)")
...
class SessionEnrollment(BaseModel, TimestampMixin):
    ...
    status = fields.CharField(
        max_length=20,
        default="enrolled",
        description="Statut: enrolled, completed, cancelled, no_show"
    )
    ...
    payment_status = fields.CharField(
        max_length=20,
        default="pending",
        description="Paiement: pending, paid, refunded"
    )
